...
public class TikaDocParser {
	...
	public static void generate(FsSettings fsSettings, InputStream inputStream, String filename, Doc doc,
		...
		// create new stream to use with default OCR if custom OCR fails
		String fullFileName = doc.getFile().getUrl().replace("file://", "");
		...
		// If custom OCR fails to extract text and the original inputStream will close
		// automatically...
		if (fsSettings.getFs().isIndexContent()) {
			try {
				...
				if (fsSettings.getFs().customOcrEnabled()) {
					if (!fsSettings.getFs().getCustomOcrSubscriptionKey().isEmpty()
							&& fsSettings.getFs().getCustomOcrSubscriptionKey() != null) {
						logger.debug("Getting custom ocr from factory");
						customOCRParser = CustomOCRFactory.getCustomOCR(fsSettings.getFs().getCustomOcrProviderName());
						logger.debug(
								"Custom OCR enabled, OCR Provider : " + fsSettings.getFs().getCustomOcrProviderName());
						...
						if (!response.isUseDefaultParser()) {
							...
							useDefault = false;
							logger.debug("Getting final response");
				...
				if (useDefault) {
					// check if originalInputStream close?
					try {
						inputStream.available();
					} catch (IOException e) {
						logger.debug("The stream has been closed, creating same stream again");
						inputStream = getDigestInputStream(in, messageDigest, fsSettings);
					}
			...
			setMeta(filename, metadata, TikaCoreProperties.MODIFIED, doc.getMeta()::setDate,
					FsCrawlerUtil::localDateTimeToDate);
			setMeta(filename, metadata, TikaCoreProperties.KEYWORDS, doc.getMeta()::setKeywords,
			...
			setMeta(filename, metadata, TikaCoreProperties.IDENTIFIER, doc.getMeta()::setIdentifier,
					Function.identity());
			setMeta(filename, metadata, TikaCoreProperties.CONTRIBUTOR, doc.getMeta()::setContributor,
					Function.identity());
			...
			setMeta(filename, metadata, TikaCoreProperties.CREATOR_TOOL, doc.getMeta()::setCreatorTool,
					Function.identity());
			...
			setMeta(filename, metadata, TikaCoreProperties.DESCRIPTION, doc.getMeta()::setDescription,
					Function.identity());
			setMeta(filename, metadata, TikaCoreProperties.CREATED, doc.getMeta()::setCreated,
					FsCrawlerUtil::localDateTimeToDate);
			setMeta(filename, metadata, TikaCoreProperties.PRINT_DATE, doc.getMeta()::setPrintDate,
					FsCrawlerUtil::localDateTimeToDate);
			setMeta(filename, metadata, TikaCoreProperties.METADATA_DATE, doc.getMeta()::setMetadataDate,
					FsCrawlerUtil::localDateTimeToDate);
			...
			setMeta(filename, metadata, TikaCoreProperties.RATING, doc.getMeta()::setRating,
			...
			if (fsSettings.getFs().isRawMetadata()) {
				...
				for (String metadataName : metadata.names()) {
					...
					// fr.pilato.elasticsearch.crawler.fs.tika level to trace
	...
	private static InputStream getDigestInputStream(InputStream inputStream, MessageDigest messageDigest,
	...
	private static <T> void setMeta(String filename, Metadata metadata, Property property, Consumer<T> setter,
			Function<String, T> transformer) {
		String sMeta = metadata.get(property);
		try {
			setter.accept(transformer.apply(sMeta));
		} catch (Exception e) {
			logger.warn("Can not parse meta [{}] for [{}]. Skipping [{}] field...", sMeta, filename,
					property.getName());
		}
	}
